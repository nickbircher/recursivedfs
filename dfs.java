   ...
   private boolean moveDFS(Point pos, GridValues[][] grid, List<Point> path)
   {
         ...
         grid[pos.y][pos.x] != GridValues.SEARCHED)
      {
      	...
      	if (grid[pos.y][pos.x] == GridValues.GOAL) {
             ...
             grid[pos.y][pos.x] = GridValues.SEARCHED;  // set current to searched

             // new points to check in recursive call
             ...
             // recursive call to copy search from lab description
             found = moveDFS(right, grid, path) || moveDFS(left, grid, path) || moveDFS(bottom, grid, path) || moveDFS(top, grid, path);
        }
          // each recursive call will chain back adding to path
          if (found) {
             this.path.add(pos);
          }
